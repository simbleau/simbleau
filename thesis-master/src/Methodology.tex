...
Once a developer has something to measure, a benchmark is written in the form of a function closure that returns \code{Measurements}, a data structure that collects \code{Measurable} trait objects. This closure is encapsulated in a \code{BenchmarkFn}, preserving the behavior and measurements, but automatically annotating GPU tracers around the closure, compatible with NVIDIA\copyright's \textit{Tools Extension SDK}, further referred to as \emph{NVTX}. In simpler terms, \code{BenchmarkFn} is synonymous with a function closure with GPU \textit{NVTX} annotations. When a binary executes a \code{BenchmarkFn} through NVIDIA\copyright tools such as NVIDIA\copyright \textit{Nsight Systems}\footnote{\href{https://developer.nvidia.com/nsight-systems}{https://developer.nvidia.com/nsight-systems}}, these GPU tracers are observed, making GPU metric sampling and integration trivial for developers.
