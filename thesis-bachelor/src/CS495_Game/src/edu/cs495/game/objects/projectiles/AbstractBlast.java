...
/**
 * An abstract blast spell
 * 
 * @version February 2019
 * @author Spencer Imbleau
 */
public abstract class AbstractBlast extends AbstractProjectile {
	
	/** The depth of the object's hitbox on the y axis */
	private static final int OBJECT_DEPTH = 4;

	/** The default speed of a blast */
	protected static final int DEFAULT_SPEED = 150;

	/** The static blast shadow for a blast spell */
	protected static final Image STATIC_BLAST_SHADOW = new Image("/equipment/spells/blast_shadow.png");
	...
	protected FilteredImage blastShadow;
	/** The blast shadow alpha filter */
	protected AlphaFilter bsAlphaFilter;

	/** The sprite sheet for this blast spell */
	protected FilteredSpriteSheet blastSpriteSheet;
	/* The width of the blast tile */
	protected int tileWidth;
	/** The height of the blast tile */
	 ...
	 * @param theta                - the angle to travel in radians
	 ...
	 * @param sender - the player who shot this projectile
	 */
	public AbstractBlast(String blastSpriteSheetPath, int tileWidth, int tileHeight, int bloomRGB, double theta,
			int speed, float ttl, Point origin, Player sender) {
		super("*BLAST*", origin.x, origin.y, tileWidth, tileHeight, OBJECT_DEPTH, sender);
		this.totalFrames = 0;
		this.frame = 0;
		this.spriteSheetPath = blastSpriteSheetPath;
		...
		// Set physics
		SATCircle hitCircle = new SATCircle(this, tileWidth / 2, tileWidth / 2, tileWidth / 2);
	...
	public void init(GameDriver gameDriver) {
		...
		this.blastSpriteSheet = new FilteredSpriteSheet(spriteSheetPath, tileWidth, tileHeight);

		// Initialize animation frame
		this.totalFrames = blastSpriteSheet.getSheetWidth() / blastSpriteSheet.getTileWidth();
		this.frame = 0;
		...
		this.offX = (int) posX - (blastSpriteSheet.getTileWidth() / 2);
		this.offY = (int) posY - (blastSpriteSheet.getTileHeight() / 2 + Player.SHOOT_HEIGHT);
		...
		this.bsAlphaFilter = new AlphaFilter();
		// Set shadow of blast to 40% less opaque as the blast itself
		this.bsAlphaFilter.setAlphaPercent(alphaFilter.getAlphaPercent() - 40);

		// Initialize projectile rotation filter
		this.rotationFilter = new RotationFilter(new Image(tileWidth, tileHeight, blastSpriteSheet.get(frame, 0)), // Img
				theta, // Initial Angle
				new Point(tileWidth / 2, tileHeight / 2)); // Anchor point = center

		// Initialize a blast shadow using the static shadow's info
		this.blastShadow = new FilteredImage(STATIC_BLAST_SHADOW.getWidth(), STATIC_BLAST_SHADOW.getHeight(),
				STATIC_BLAST_SHADOW.getPixels().clone());

		// Apply and build filters
		this.blastSpriteSheet.addFilter(this.rotationFilter);
		this.blastSpriteSheet.addFilter(this.alphaFilter);
		this.blastSpriteSheet.buildAll();

		this.blastShadow.addFilter(bsColorFilter);
		this.blastShadow.addFilter(this.bsAlphaFilter);
		this.blastShadow.build();
	}
	...
	public void update(GameDriver gameDriver) {
		...
		// Update frame
		this.frame = (int) ((gameDriver.getGameTime() / (GameDriver.NANO_SECOND / totalFrames)) % totalFrames); // Loop
																											// animation
		...
		double iTheta = rotationFilter.getTheta();
		double fTheta = (iTheta + (GameDriver.UPDATE_DT / 10 * speed)) % (2 * Math.PI);
		this.rotationFilter.setTheta(fTheta);
		this.blastSpriteSheet.clean(frame, 0);
		this.blastSpriteSheet.build(frame, 0);
		...
		if (gameDriver.getLevel().hasLighting()) {
			...
			// Rebuild filter
			this.bsAlphaFilter.setAlphaPercent(opacity);
			this.blastShadow.clean();
			this.blastShadow.build();
		} else {
			// Set opacity to default
			if (bsAlphaFilter.getAlphaPercent() != 100 - shadowTaxPercent) {
				this.bsAlphaFilter.setAlphaPercent(100 - shadowTaxPercent);
				this.blastShadow.clean();
				this.blastShadow.build();
			}
		}
	...
	public void render(GameDriver gameDriver, Renderer renderer) {
		...
		// Draw Shadow
		renderer.draw(blastShadow, Integer.MIN_VALUE, (int) offX, (int) posY - STATIC_BLAST_SHADOW.getHalfHeight());
		...
		// Draw
		renderer.draw(blastSpriteSheet, frame, 0, (int) posY, (int) offX, (int) offY);

//		// Draw physics
//		if (DeveloperLog.isDebugging()) {
//			if (hasPhysics()) {
//				renderer.draw(
//						new Box(getPhysics().getCollisionArea().getXMax() - getPhysics().getCollisionArea().getXMin(),
//								getPhysics().getCollisionArea().getYMax() - getPhysics().getCollisionArea().getYMin(),
//								0xff00ffff, true),
//						(int) posY, offX + getPhysics().getCollisionArea().getXMin(),
//						offY + getPhysics().getCollisionArea().getYMin());
//			}
//		}
	}
}
